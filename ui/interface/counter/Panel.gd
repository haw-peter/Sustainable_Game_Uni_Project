...
#func _ready():
	...
	#if interface_node:
		#interface_node.connect("gold_count_changed", Callable(self, "_on_gold_count_changed"))
		#
#
#func _process(delta) -> void:
	...
	## Check if it's time to increment gold
		...
		#$"/root/Main/Camera2D/Interface".gold_count += gold_increment_amount
		## Update gold label or perform any necessary actions
		#
...
#func get_time_formatted() -> String:
	...
	#
## Function to handle the gold count change
#func _on_gold_count_changed(new_count: String) -> void:
	## Update the gold label in the panel
	#$Counters/GoldCounter/Number.text = str(new_count)
	#$Counters/GoldCounter/Number/AnimationPlayer.play("shake")
	#
#func update_gold_label(gold_count: int) -> void:
	#print("Updating gold label: ", gold_count)
	#$"/root/Main/Camera2D/Interface/Counters/GoldCounter/Number".text = str(gold_count)
	## Add any additional actions related to updating the gold label here	
		##################################################################
extends Panel
