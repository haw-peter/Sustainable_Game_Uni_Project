...
func _ready():
	...
	var interface_node = get_parent().get_parent() #get_node("Camera2D/Interface")
	if interface_node:
...
func _update_gold_label(gold_count):
	var gold_counter = self.get_child(0)
	if gold_counter:
...
func _update_wood_label(wood_count):
	var wood_counter = self.get_child(0) #$Counters/WoodCounter/Number
	if wood_counter:
...
func _update_house_label(house_count):
	var house_counter = self.get_child(0) #$Counters/HouseCounter/Number
	if house_counter:
